...
class CurrentMonthPredictor:
    ...
    def prepare_training_data(self, lookback_months=3):
        """Fetch historical usage from daily_usage and return Prophet-ready dataframe"""
        ...
        try:
            with engine.connect() as conn:
                ...
                print(f"DEBUG: Found {len(df)} rows for {self.msisdn}")
    ...
    def _zero_days_result(self, usage):
        return {
            'msisdn': str(self.msisdn), 'current_usage_gb': round(usage, 2),
...
def predict_for_subscriber(msisdn, retrain=False):
    ...
    if not summary:
        logger.warning(f"No usage summary found for {msisdn}")
        return None
    ...
    if retrain or not predictor.load_model():
        if not predictor.train(lookback_months=3):
            logger.error(f"Failed to train model for {msisdn}")
            return None
